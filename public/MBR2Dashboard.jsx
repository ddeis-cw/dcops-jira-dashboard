...
// ── Constants ─────────────────────────────────────────────────────────────────
const PROJECTS = [
  { key:"all", label:"All Projects",             short:"ALL", color:"#475569" },
...
function momArrow(pct, delta, prevBase, positiveIsGood = false) {
  if (pct === null || prevBase == null) return { icon:"—", color:"#94a3b8" };
  const up   = pct > 0;
  const good = positiveIsGood ? up : !up;
  const col  = good ? "#22c55e" : "#ef4444";
  const sign = up ? "+" : "";
  // When base is tiny, the % is technically correct but needs context
  if (prevBase < 10 && Math.abs(pct) > 200) {
    return { icon: `${sign}${pct.toLocaleString()}% (from ${prevBase})`, color: col };
  }
  return { icon: `${up?"▲":"▼"} ${sign}${pct.toLocaleString()}%`, color: col };
}
...
function SiteRow({ site, curr, prev, mom_pct, mom_delta, rank }) {
  ...
  return (
    <tr style={{ borderBottom:"1px solid #ffffff" }}>
      <td style={{ padding:"8px 10px", color:"#94a3b8", fontSize:11, width:28 }}>#{rank}</td>
      <td style={{ padding:"8px 10px", fontWeight:700, color:"#1e293b", fontSize:12 }}>{site}</td>
      <td style={{ padding:"8px 10px", textAlign:"center", color:"#0f172a", fontWeight:600 }}>{curr.total.toLocaleString()}</td>
      ...
      <td style={{ padding:"8px 10px", textAlign:"center", color:"#94a3b8", fontSize:10 }}>
...
export default function MBR2Dashboard() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f8fafc", color:"#1e293b", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", padding:"20px 24px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:24, fontWeight:700, color:"#1e293b" }}>MBR Dashboard v2</div>
          <div style={{ fontSize:12, color:"#94a3b8", marginTop:2 }}>Monthly Business Review — Local DB · {summary?.total?.toLocaleString() || "—"} tickets loaded</div>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <select value={selMonth.from} onChange={e => setSelMonth(MONTHS.find(m=>m.from===e.target.value)||MONTHS[0])}
            style={{ fontSize:12, padding:"6px 12px", borderRadius:8, border:"1px solid #e2e8f0", background:"#ffffff", color:"#0f172a", cursor:"pointer" }}>
          ...
          <button onClick={fetchAll} disabled={loading}
            style={{ padding:"6px 16px", fontSize:12, fontWeight:600, borderRadius:8, border:"none", background: loading ? "#94a3b8" : col, color:"#fff", cursor: loading ? "not-allowed" : "pointer" }}>
            {loading ? "Loading…" : "↻ Refresh"}
          </button>
        </div>
      </div>

      {error && <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:12 }}>⚠ {error}</div>}

      {loading && <div style={{ textAlign:"center", padding:"60px 0", color:"#94a3b8", fontSize:13 }}>Fetching data from local DB…</div>}
      ...
      {!loading && summary && (<>
          ...
          <div style={{ background:"#ffffff", borderRadius:10, padding:"16px", border:"1px solid #e2e8f0", marginBottom:20 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:6, marginTop:10 }}>
              {trends.months.map(m => {
                ...
                return (
                  <div key={m.month} style={{ background:"#f8fafc", borderRadius:6, padding:"8px 10px" }}>
                    ...
                    {m.avg_mttr_hours && <div style={{ fontSize:9, color:"#94a3b8" }}>MTTR {fmtHours(m.avg_mttr_hours)}</div>}
                  </div>
                );
              })}
            </div>
                ...
                <thead>
                  <tr style={{ background:"#f8fafc" }}>
                    {["#","Site","Total","Closed","Close %","On Hold","MTTR","MoM","Prev Closed"].map(h => (
                      <th key={h} style={{ padding:"8px 10px", textAlign: h==="Site"||h==="#" ? "left" : "center", color:"#64748b", fontSize:10, fontWeight:600, borderBottom:"1px solid #e2e8f0", whiteSpace:"nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
          ...
          <div style={{ background:"#ffffff", borderRadius:10, border:"1px solid #e2e8f0", padding:"16px", marginBottom:20 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:8 }}>
              {statusTime.statuses.filter(s=>s.avg_hours).map(s => {
                ...
                return (
                  <div key={s.status} style={{ background:"#f8fafc", borderRadius:8, padding:"10px 12px" }}>
                    <div style={{ fontSize:10, color:"#94a3b8", marginBottom:4, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{s.status}</div>
                    <div style={{ fontSize:20, fontWeight:700, color:c }}>{fmtHours(h)}</div>
                    <div style={{ fontSize:9, color:"#94a3b8" }}>{s.n.toLocaleString()} tickets</div>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ borderTop:"1px solid #ffffff", paddingTop:16 }}>
          <div style={{ fontSize:11, color:"#94a3b8", marginBottom:8, fontWeight:600 }}>Also view: other projects</div>
